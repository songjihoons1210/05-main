...
function func1() {
    const id = document.querySelector('#id'); console.log(id);    
    const userid = id.value;
    console.log(userid);
    const pass = document.querySelector('#pass'); console.log(pass);
    console.log(pass);
    localStorage.setItem( 'userid' , userid , pass);
}

//

const userSettings = { theme: 'dark', notifications: true, language: 'ko' };
localStorage.setItem( 'idid' , JSON.stringify( {theme : '다크' , notifications : true , language : '한국어' } ) );

console.log( localStorage.getItem( 'idid') );


//

let thems = localStorage.getItem('idid');
let jso = JSON.parse(thems);
console.log(jso);


//

// localStorage.removeItem('userid'); // 삭제 i저장
