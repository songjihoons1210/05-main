...
//[3]객체 선언시 속성구성, {속성명 : 자료}
// 속성명 과 자료의 한쌍하고 , (쉼표) 구분하여 여러개 쌍(entry)를 { } 로 감싼다. 
...
// [4] 객체 호출
// 변수명 : 전체 호출   : 전체 호출
// 변수명.속성명        : 특정한 속성의 자료 호출 * .(접근/도트 연산자) 특정한 위치(속성)은 이동한다는 뜻
// 변수명['속성명'] : 특정한 속성의 자료 호출 * 속성명이 특수문자포함 또는 변수일떄
// Object.keys( 변수명) : 객체내 모든 속성명(key) 를 배열로 반환
// Object.values( 변수명)  : 객체내 모든 자료(value) 를 배열로 반환
// oBJECT.ENTRIES( 변수명)    : 객체내 모든 엔트리(entey) 를 배열로 반환
console.log(obj2);
...
// [6] 객체내 속성 수정 , 변수명.속셩명 = 새로운자료 , 변수명['속성명'] = 새로운자료
obj2.속성명1 = 20;
...
console.log(var1)
// [9] in 연산자 , '찾을속성명' in 변수명 , 객체내 특정한 속성이 있는지 확인(trye/false)
console.log('props1' in var2); //true
console.log('props6' in var2); //false

// [10] 배열 과 객체 조합
// 예시1 공통점 : 여러개 자료들을 하나의 자료로 묶는다.
// 예시1 차이점 : 배열(인덱스으로 자료 구분) vs 객체(속성명으로 자료 구분 ) 
// - 동일한 자료자료의 의미를 가질때는 배열 , 서로 다른 자료의 의미를 가질때는 객체
// ['콜라' , 1000] -> {}   -> 배열보다는 객체 권장!!
const 제품1 = { '제품명' : '콜라'  , '가격' : 1000 }
const 제품2 = { '제품명' : '사이다' , '가격' : 2000}
...
for(let index = 0; index <= 제품목록.length -1 ; index++){
    const product = 제품목록[index]; // index번쨰의 제품(객체)
    console.log(product.제품명 ); // index번쨰의 제품의 속성값 호출
