...
/*
예제 1: 자기소개 객체 만들기
자신의 이름(name), 나이(age), 사는 곳(city)을 속성으로 갖는
 person이라는 이름의 객체를 만들고, console.log()로 객체 전체를 출력하시오.
 
*/


const person = { 이름: "송지훈", 나이: 40, 위치: "인천" };
console.log(person);

/*
예제 2: 객체의 값(Value) 출력하기
다음과 같은 book 객체가 있습니다. 반복문을 사용하여
 이 객체의 모든 값 을 하나씩 콘솔에 출력하시오.
*/
const book = {
...
person1.email = 'test@naver.com'; //이메일 추가 
delete person1.age;
console.log(person1);
...
console.log(` 합계 :  ${product.price * product.stock}`);

/*예제 5: 대괄호 표기법 사용하기
다음 user 객체가 있습니다. 대괄호 
표기법을 사용하여 job-title 속성의 값('개발자')을 콘솔에 출력하시오.
*/
...
console.log(" 학생이 사는 도시는", student.address.city);


/*
예제 7: 객체에 특정 속성이 있는지 확인하기(  in 연산자 )
character 객체에 level이라는 속성이 있는지 in 연산자를 사용하여
확인하고, 결과를 true 또는 false로 콘솔에 출력하시오.
*/
...
const namea = Object.keys(inventory); // 모든 상품의 이름.
for (let i = 0; i <= namea.length - 1; i++) {
    const inven = namea[i];
    if (inventory[inven] < 10) {
        console.log(inven);
    }
}
...
for (let i = 0; i <= students.length - 1; i++) {
    const 이름 = students[i];
    console.log(`이름 : ${이름.name} | 전공 : ${이름.major}`)
    ...
    [구현 조건]
      1. 공공데이터 포털에서 '인천광역시 부평구_인구 현황' 의 
      ...
      4. 테이블의 각 셀에는 '동별', '세대수', '인구수(계)', '인구수(남)', 
      '인구수(여)' 정보가 순서대로 포함되어야 합니다.
      5. 최종적으로 완성된 HTML 테이블 문자열을 document.write() 사용하여 화면에 출력합니다.
    [ 공공데이터 open API 신청 ]
      1. 공공데이터 포털 : https://www.data.go.kr
      2. 회원가입/로그인
      3. '부평구 인구' 검색
      4. '인천광역시 부평구_인구 현황' Open API를 찾아 [활용신청] 버튼을 누르고, 
          절차에 따라 인증키를 발급받습니다.
    ...
    */
    ...
    let html = `<table>
                        <tr>
                            ...
                            <th>인수구(남)</th> <th>인구수(여) </th>
                        </tr>`

    for (let index = 0; index <= data.length - 1; index++) {
        // index는 0부터 마지막인덱스까지 1씩 증가
        const obj = data[index]; // index 번째의 객체(동별 인구정보) 호출
        //테이블 내용물
        html += `<tr>
                    <td> ${obj.동별}</td><td>${obj.세대수}</td>
                    <td> ${obj["인구수(계)"]}</td> <td>${obj["인구수(남)"]}</td>
                    <td> ${obj["인구수(여)"]}</td>
                </tr>`
    }
        html += `</table>`
